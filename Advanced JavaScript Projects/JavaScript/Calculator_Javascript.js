...
function Input_Decimal(dot) {
    ...
    of(Calculator.Wait_Second_Operand === true)
    return;
...
function Handle_Operator(Next_Operator) {
    ...
    //When an operator key i pressed we convert the current number 
    //displayed on the screen to a number and then store the result in 
    //calculator/first_operand if it doesn't already exist. 
    const Value_of_Input = parseFloat(Display_Value);
    //Checks if an operatir already exists abd uf waut secibd ioerabd us true then updates the operator and exits from the function
    ...
    if (First_Operand == null) {
        ...
        //If operator exists property lookip is perfored for the operator in the perform calculatio object
        //and the function tht matches the operator is executed
        let result = Perform_Calculation[operator](Value_Now, Value_of_Input);
        //Here we add a fixed amount of numebrs after the decimal
